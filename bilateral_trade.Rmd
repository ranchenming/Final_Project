...
rf_model <- rand_forest(mode = "regression", mtry = 3, trees = 1000) %>%
  set_engine("ranger") %>%
  fit(FLOW ~., data = bake(recipe1, new_data = bit_training))rf_model <- rand_forest(mode = "regression", mtry = 3, trees = 1000) %>%
  set_engine("ranger") %>%
...
M_d <- left_join(x= M_p, y = M3, by = "metric")
M_e <- left_join(x= M_d, y = M4, by = "metric")
...
write_csv(Metrics_com, "Metrics_com.csv")
...
final_r %>%
  ...
  xlab("Predicted Trade Flow")+
  ylab("Actual Trade Flow") +
  labs(title = "Graph 6. Random Forest Model over Entire Data") +
   theme(plot.title = element_text(hjust = 0.5),
        legend.title = element_blank(),
        legend.box.background = element_blank(),
        legend.text = element_text(size=rel(1)),
...
bind_cols(bitrade,final_r) %>%
  ...
  ggplot(aes(x = year, y = value, color = name, group = name)) +
      ...
      ggtitle("Graph 7. Yearly Actual V.S Predicted Trade Flow, 1995-2018") +
