---
title: "Predicting Bilateral Trade: Gravity Model V.S. Supervised Machine Learning"
author: "Chenming Ran"
date: "4/26/2020"
...
Population is a control variable commonly  used by Circlaeys et al (2017). Therefore, this study also incorportations country total population data from United Nations Population Division  — World Population Prospect from 1995 to 2018 [10]. 
...
```{r time-series, echo=FALSE, fig.align="center", fig.height=2, fig.width=3, message=FALSE, warning=FALSE}
line <- bitrade %>%
  group_by(year) %>%
...
```{r PA, echo=FALSE, fig.align="center", fig.height=2, fig.width=3, message=FALSE, warning=FALSE}
library(Hmisc)
...
```{r formula2, fig.align="center", fig.width= 6, fig.height= 0.5, echo=FALSE}
library(jpeg)
library(grid)
...
Random Forest model works is an extension of decsion tree as it bulds multiple decision trees within random subsets of the data. Then each decision tree seperates the data into multiple sub-spaces so that the outcomes are maximumly homogeneous. random forest can handle both numerical and categorical data, which fits the bilateral trade dataset, and nonlinear relationships do not affect its performance. Although it does not capture time-series effects, the ACF test shows that actuall time-series effects are likely to be small. Hence using ML techniques incapable of addressing time-series effects is less concerned for bias of comparison. 
...
Accordingly, I trained 4 different models discussed above, with 3 different recipes addressing major model assumptions. For estimating KNN model, I follow the conventional determination of k and choose the value to be 588, which is the square root of sample number in my training dataset.
...
I estimate the in-sample and out-of-sample error rates by predicting values in both training and testing data set. Among all models, the random forest model has the best performance over training and test datasets. According to the Table 1 below, most in-sample and out-of-sample errors are close for each model except for the random forest model and KNN model. This means a hold-out validation using 30% of the entire dataset is fairly efficient. The reason is that my updated dataset does not have as large sample as Circlaeys' study. A k-fold cross validation should decrease the chance of overfitting.
...
```{r estimate the in-sample error, include=FALSE}
...
M4 <- bind_cols(
  ...
  metrics(truth = FLOW, estimate = .pred) %>%
  rename (KNN_model_1 = .estimate, metric = .metric) %>%
  select(-.estimator)
...
```{r estimate the out-of-sample error, include=FALSE}
#Estimate the out-of-sample error rate by predicting values in the testing data set.
Metric1 <- bind_cols(
...
```{r print the in-sample errors, echo=FALSE}
M_p <- left_join(x = M1, y = M2, by = "metric") 
M_d <- left_join(x= M_p, y = Metric3, by = "metric")
M_e <- left_join(x= M_d, y = Metric4, by = "metric")
M <- as.data.frame(t(M_e)) 
M <- data.frame(row.names(M), M, row.names = NULL) #change index into column
names(M) = as.character(unlist(M[1,])) #change first row into column names
M <- M[-1,] %>%
   rename(Model = metric, RMSE_train = rmse, Rsquared_train= rsq, MAE_train = mae) 
rownames(M) <- NULL

rm(M_p, M_d, M_e, M1, M2, M3, M4)
```

```{r print the out-of-sample errors, echo=FALSE}
Metrics_p <- left_join(x = Metric1, y = Metric2, by = "metric") 
Metrics_d <- left_join(x= Metrics_p, y = Metric3, by = "metric")
Metrics_e <- left_join(x= Metrics_d, y = Metric4, by = "metric")
Metrics <- as.data.frame(t(Metrics_e)) 
...
Metrics <- Metrics[-1,] %>%
   rename(Model = metric, RMSE_test = rmse, Rsquared_test= rsq, MAE_test = mae) 
rownames(Metrics) <- NULL

Metrics_com <- left_join(x = M, y = Metrics, by = "Model") 

kable(Metrics_com, format = "markdown", caption = " Model Performances")

rm(Metrics_p, Metrics_d, Metrics_e, Metric1, Metric2, Metric3, Metric4, M)
rm(split, bit_training)
```
...
As for the first KNN model, there are large increase in RMSE and MAE and decrease in R-squared when predicting values in testing dataset, which means the extent of overfitting is large. Thus, The KNN model is not likely to be more effective than linear regression model. I recognize a 10 fold cross-validation may increase the accuracy of KNN, but the random forest certainly performs much better among alll mondels. Below are scatter plots that demonstrate the goodness of fit for each model.

```{r scatter test, include=FALSE}
t_1 <- bind_cols(
  bake(recipe1, new_data = bitrade_testing),
  predict(lm_model, bake(recipe1, new_data = bitrade_testing)))
...
t_2 <- bind_cols(
  bake(recipe2, new_data = bitrade_testing),
  predict(lm_AR_model, bake(recipe2, new_data = bitrade_testing)))
...
t_3 <- bind_cols(
  bake(recipe1, new_data = bitrade_testing),
  predict(rf_model, bake(recipe1, new_data = bitrade_testing)))
write.csv(t_3, "t3.csv")
...
t_4 <- bind_cols(
  bake(recipe3, new_data = bitrade_testing),
  predict(knn_model, bake(recipe3, new_data = bitrade_testing)))
write.csv(t_4, "t4.csv")
...
```{r make prediction on entire data, include=FALSE}
#Linear prediction
final_1 <- bind_cols(
  bake(recipe1, new_data = bitrade),
  predict(rf_model, bake(recipe1, new_data = bitrade)))
...
A scatter plot of the predicted vs. actual prices shows residuals with a some outliers at the right side, which are not well explained by the model. The predictions are more accurate for country pairs with smaller trade flows relative to those having large bilateral trade flow. Nevertheless, the model shows fairly good level of robustness.
```{r scatter-plot 2, fig.height=2, fig.width=3, echo=FALSE, warning=FALSE}
...
final_r %>%
  ...
  xlab("Predicted Trade Flow")+
  ylab("Actual Trade Flow") +
  labs(title = "Graph 4. Random Forest Model over Entire Data") +
   theme(plot.title = element_text(hjust = 0.5),
        legend.title = element_blank(),
        legend.box.background = element_blank(),
        legend.text = element_text(size=rel(1)),
...
```{r performance, echo=FALSE, fig.height=2, fig.width=3, warning=FALSE}
...
bind_cols(bitrade,final_r) %>%
  ...
  ggplot(aes(x = year, y = value, color = name, group = name)) +
      ...
      ggtitle("Graph 5. Yearly Actual V.S Predicted Trade Flow, 1995-2018") +
